...
const ReviewProductModal = ({ visible, product, onCancel }) => {
    ...
    // Fetch reviews for the product when modal becomes visible
    useEffect(() => {
    ...
    const calculateAvgRating = (data) => {
        const totalRatings = data.count5Star + data.count4Star + data.count3Star +
            data.count2Star + data.count1Star;
        const weightedSum = (5 * data.count5Star) + (4 * data.count4Star) +
            (3 * data.count3Star) + (2 * data.count2Star) +
            (1 * data.count1Star);
    ...
    const handleReply = async () => {
        if (!replyForm.content.trim()) {
            notification.warning({
                message: 'Thông báo',
                description: 'Phản hồi không được để trống',
            });
            return;
        }
        ...
        setLoading(true);
        try {
            await reviewService.reply_product_review(replyForm.reviewId, replyForm.content);

            // Update reviews list after successful reply
            setReviews(
                reviews.map((review) =>
            ...
            notification.success({
                message: 'Thành công',
                description: 'Đã phản hồi đánh giá thành công',
            });
        } catch (error) {
            notification.error({
                message: 'Lỗi',
                description: `Không thể phản hồi: ${error.message}`,
            });
        } finally {
            setLoading(false);
        }
        ...
        >
            <Spin spinning={loading}>
                <Tabs defaultActiveKey="reviews">
                    <TabPane
                        tab={<span><StarOutlined />Tổng quan đánh giá</span>}
                        key="overview"
                    >
                                ...
                                <Space direction="vertical" style={{ width: '100%' }}>
                                    {[5, 4, 3, 2, 1].map(star => (
                                        <div key={star} className="flex items-center">
                                            ...
                                            <Progress
                                                percent={calculateRatingPercentage(reviewStats[`count${star}Star`])}
                                                showInfo={false}
                    ...
                    <TabPane
                        tab={<span><MessageOutlined />Danh sách đánh giá</span>}
                        key="reviews"
                    >
                        <div className="p-4">
                                                    ...
                                                    <div className="flex items-start space-x-4">
                                                        ...
                                                        <div className="flex-1">
                                                            <div className="flex items-center mb-1">
                                                                <Text strong className="mr-2">{review.customer_id?.fullName || 'Khách hàng'}</Text>
                                                            ...
                                                            <div className="mt-2">
                                                                {review.title && <Text strong className="block mb-1">{review.title}</Text>}
                                                                    ...
                                                                    {review.images.map((img, idx) => (
                                                                        <div key={idx}>
                                                                            <Image
                                                                                ...
                                                                                alt={`Hình ảnh ${idx + 1}`}
                                                                                preview={{
                                                                                    visible: previewVisible && previewImage === img,
                                                    ...
                                                    {review.reply ? (
                                                        <div className="bg-gray-50 p-4 rounded-lg">
                                                            <div className="flex items-start space-x-3">
                                                                <Avatar style={{ backgroundColor: '#1890ff' }}>Shop</Avatar>
                                                                <div className="flex-1">
                                                                    <div className="flex items-center">
                                                                        <Text strong className="mr-2">Phản hồi của shop</Text>
                                                                        <Text type="secondary" className="text-xs">
                                                                            {formatDate(review.reply.replied_at)}
                                                                        </Text>
                                                                    </div>
                                                                    <Paragraph className="mt-2">{review.reply.content}</Paragraph>
                                                                    <Button
                                                                        type="link"
                                                                        size="small"
                                                                        className="p-0"
                                                        ...
                                                        <div>
                                                            {replyForm.reviewId === review._id ? (
                                                                <Form layout="vertical">
                                                                    ...
                                                                    <Form.Item>
                                                                        <Space>
                                                                            ...
                                                                            <Button
                                                                                onClick={() => setReplyForm({ reviewId: null, content: '' })}
                                                                            >
                                                                                Hủy
                                                                            </Button>
                                                                        </Space>
                                                                ...
                                                                <Button
                                                                    ...
                                                                    onClick={() => setReplyForm({ reviewId: review._id, content: '' })}
                                                                >
                                                                    Phản hồi
                                                                </Button>
                                                            )}
