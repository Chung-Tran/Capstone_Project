...
const ProductForm = ({ isOpen, onClose, refreshData, initialData = {}, mode = 'add' }) => {
    ...
    useEffect(() => {
        if (isOpen && initialData && Object.keys(initialData).length > 0) {
            form.setFieldsValue({
                ...initialData,
                category_id: initialData.category_id?.map(cat => cat._id || cat) || [],
    ...
    const handleSubmit = async () => {
        try {
            ...
            Object.keys(values).forEach(key => {
                ...
                if (key === 'dimensions' && values[key]) {
                    Object.keys(values[key]).forEach(dimKey => {
                        if (values[key][dimKey] !== undefined && values[key][dimKey] !== null) {
                ...
                else if (values[key] !== undefined && values[key] !== null && key !== 'main_image' && key !== 'additional_images') {
            ...
            if (isEditing) {
                ...
                const removedImageUrls = initialData.additional_images?.filter(initialUrl =>
                    !keptImageUrls.includes(initialUrl)
                ) || [];
            ...
            let response;
            if (isEditing) {
                response = await productService.product_update(initialData._id, formData);
        ...
        return (
            ...
            >
                <Tabs defaultActiveKey="1">
                    <TabPane tab="Thông tin cơ bản" key="1">
                        <Descriptions bordered column={1}>
                            ...
                            <Descriptions.Item label="Trạng thái">
                                <Badge
                                    status={initialData.status === "active" ? "success" : "default"}
                                    text={initialData.status === "active" ? "Đang bán" : "Tạm ngừng"}
                                />
                            </Descriptions.Item>
                            <Descriptions.Item label="Danh mục">
                                {initialData.category_id && initialData.category_id.length > 0
                                    ? initialData.category_id.map((cat) => cat.name || "Không xác định").join(", ")
                                    : "-"}
                            </Descriptions.Item>
                    ...
                    <TabPane tab="Hình ảnh và mô tả" key="3">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                {initialData.additional_images && initialData.additional_images.length > 0 ? (
                        ...
                        <div>
                            <div className="prose max-w-full" dangerouslySetInnerHTML={{ __html: initialData.description || "Không có mô tả" }}></div>
                        </div>
                    </TabPane>
                </Tabs>
            </Modal>
